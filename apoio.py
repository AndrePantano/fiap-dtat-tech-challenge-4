...
class Apoio:
    ...
    def gerar_df_longo_app(self):
        ...
        df_longo = df_longo.loc[df_longo.cd_variavel != "smoke"]

        # nr_categoria de Always para Frequently
        # ds_categoria de Sempre para Frequentemente	
        df_longo.loc[
            (df_longo['cd_variavel'] == 'calc') & 
            (df_longo['ds_categoria'] == 'Sempre'),
            ['nr_categoria', 'ds_categoria']
        ] = ['Frequently', 'Frequentemente']


        # Filtro
        filtro = df_longo['cd_variavel'] == 'mtrans'

        # Mapeamento nr_categoria
        map_nr = {
            'Automobile': 'Passive',
            'Bike': 'Active',
            'Motorbike': 'Passive',
            'Public_Transportation': 'Passive',
            'Walking': 'Active'
        }

        # Mapeamento ds_categoria
        map_ds = {
            "Carro":'0',
            "Moto":'0',
            "Bicicleta":'1',
        ...
        # Aplicando apenas nas linhas filtradas
        df_longo.loc[filtro, 'nr_categoria'] = (
            df_longo.loc[filtro, 'nr_categoria'].map(map_nr)
        )

        df_longo.loc[filtro, 'ds_categoria'] = (
            df_longo.loc[filtro, 'ds_categoria'].map(map_ds)
        )

        df_longo.loc[filtro, 'sk_categoria'] = (
            df_longo.loc[filtro, 'nr_categoria']
            .eq('Passive')
            .astype(int)
            .replace({1: 1, 0: 2})
        ...
        df_longo.loc[(df_longo['cd_variavel'] == 'mtrans'),['nr_categoria']] = ['mtrans_ativo']
